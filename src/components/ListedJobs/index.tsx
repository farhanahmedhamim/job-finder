import { Bookmark, BookmarkOutline, BusinessOutline, LocationOutline, NewspaperOutline } from "react-ionicons";
...
interface ListedJobsProps {
    jobs: Job[];
    setSevedJobs: React.Dispatch<React.SetStateAction<number[]>>;
    savedJobs: number[];
}

const ListedJobs = ({ jobs, setSevedJobs, savedJobs }: ListedJobsProps) => {
    const navigate = useNavigate();
    const handleSave = (id: number) => {
        const item = savedJobs.find((j) => j === id);
        if (item) {
            setSevedJobs(savedJobs.filter((j) => j !== item));
        } else {
            setSevedJobs((prev) => [...prev, id]);
        }
    };
    ...
    return (
        <div className="w-full flex items-center justify-between flex-wrap mt-8">
            ...
            {jobs.map((job) => {
                return (
                    <div key={job.id} className="md:w-[49%] w-full bg-white mb-5 rounded-lg flex items-center justify-between md:p-8 py-8 px-4 border border-gray-200">
                        ...
                        <div className="flex flex-col gap-4 self-end">
                            ...
                            <div
                                ...
                                className={`flex items-center gap-2 cursor-pointer rounded-md justify-center py-1 border border-gray-200 ${savedJobs.some((j) => j === job.id) ? "bg-gray-100" : "bg-gray-200"}`}>
