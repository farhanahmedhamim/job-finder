            ...
            <div className="w-full md:px-[50px] px-[23px] flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-6">
                    {navLinks.map((navlink) => {
                        return (
                            <a href={navlink.path} key={navlink.title} className={`font-medium text-[15px] ${navlink.active ? "text-indigo-500" : "text-[#828ea8]"} transition-all duration-200 hover:text-indigo-500`}>{navlink.title}</a>
                        )
                    })}
                </div>
            </div>
